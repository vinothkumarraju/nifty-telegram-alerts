...
SCAN_INTERVAL_SECONDS = 60  # Check market every 60 seconds
...
def send_telegram_error(err_title: str, err_detail: str):
    ...
    msg = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔄 *Status:* Auto-retrying in 60 seconds."
    )
    send_telegram(msg)
...
def find_first_swing_pivot(df_5m, direction="BULLISH", max_lookback=30):
    """Refined backward-scan fractal pivot detector."""
...
def evaluate_and_notify():
    ...
    # 1. Pre-Market Session Update (09:08 - 09:14 AM)
    ...
    print(f"[{current_time_str}] Spot: {spot:.2f} | 5 EMA: {e5:.2f} | 10 EMA: {e10:.2f} | Gap: {live_gap:.2f} pts")
    ...
    # ==========================================================
    # 2. 1H CANDLE CLOSE AUDIT & CONFIRMATION / ROLLBACK
    # ==========================================================
    ...
    # ==========================================================
    # 3. 09:15 AM OPENING GAP MOMENTUM ENTRY
    # ==========================================================
    ...
    # ==========================================================
    # 4. 03:25 PM EOD CROSSOVER HOLD (BTST / STBT)
    # ==========================================================
    if hour == 15 and minute == 25:
        is_live_bull = e5 > e10
        is_live_bear = e5 < e10
        req_eod_type = "BULL_CALL_SPREAD" if is_live_bull else "BEAR_PUT_SPREAD"

        if (is_live_bull and state.get("last_cross_state") != "BULL") or (is_live_bear and state.get("last_cross_state") != "BEAR"):
            if state.get("active_position") is None or state["active_position"]["type"] != req_eod_type:
                exit_block, spread_name, b_strike, s_strike = execute_spread(
                    req_eod_type, spot, target_expiry, current_time_str, state, reason="1525_EOD_CROSSOVER_HELD", prev_close=prev_close
                )
                state["last_cross_state"] = "BULL" if is_live_bull else "BEAR"
                state["last_traded_1h_candle"] = closed_1h_time
                save_state(state)

                eod_msg = (
                    f"🎯 *03:25 PM EOD CLOSING CROSSOVER EXECUTED (BTST/STBT)*\n"
                    f"━━━━━━━━━━━━━━━━━━━━━\n"
                    f"{exit_block}"
                    f"📦 *Position:* `{spread_name}`\n"
                    f"📅 *Target Expiry:* `{target_expiry}`\n"
                    ...
                    f"📍 *Spot:* `{spot:.2f}` {diff_str}\n"
                    f"━━━━━━━━━━━━━━━━━━━━━\n"
                    f"🌙 *Action:* 03:25 PM cross held. Opened overnight spread."
                )
                send_telegram(eod_msg)
                return

    # ==========================================================
    # 5. INTRADAY 1H CROSSOVER & 5M CANDLE CLOSE BREAKOUT
    # ==========================================================
    if (hour < 15 or (hour == 15 and minute < 15)) and (state.get("last_traded_1h_candle") != closed_1h_time):
        ...
        # Execute on 5M Completed Candle Breakout
...
def run_live_loop():
    print("🚀 Nifty Live Continuous Scanner & Paper Trader Initialized.")
    now_ist = datetime.now(IST)
    startup_msg = (
        ...
        f"⚙️ *Mode:* `Live 5M Candle Close + 1H Confirmation`\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"🤖 Bot is now actively monitoring Nifty live market."
    )
