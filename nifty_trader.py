...
def send_telegram_error(err_title: str, err_detail: str):
    ...
    msg = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔄 *Runner:* Automatic retry on next scheduled interval."
    )
    send_telegram(msg)
...
def run_single_pass():
    """Executes a single market scan cycle for GitHub Actions runner."""
    state = load_state()
    now_ist = datetime.now(IST)
    ...
    print(f"[{current_time_str}] Executing GitHub Action Nifty Strategy Engine...")
    ...
    # ==========================================================
    if hour == 15 and minute == 25:[cite: 1]
        is_live_bull = e5 > e10[cite: 1]
        is_live_bear = e5 < e10[cite: 1]
        req_eod_type = "BULL_CALL_SPREAD" if is_live_bull else "BEAR_PUT_SPREAD"[cite: 1]

        if (is_live_bull and state.get("last_cross_state") != "BULL") or (is_live_bear and state.get("last_cross_state") != "BEAR"):[cite: 1]
            if state.get("active_position") is None or state["active_position"]["type"] != req_eod_type:[cite: 1]
                exit_block, spread_name, b_strike, s_strike = execute_spread(
                    req_eod_type, spot, target_expiry, current_time_str, state, reason="1525_EOD_CROSSOVER_HELD", prev_close=prev_close[cite: 1]
                )
                state["last_cross_state"] = "BULL" if is_live_bull else "BEAR"[cite: 1]
                state["last_traded_1h_candle"] = closed_1h_time[cite: 1]
                save_state(state)
